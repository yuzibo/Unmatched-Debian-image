...
echo "confirm to execute the last"

make_imagefile()
{
    if [ -f debian-release ]; then
        echo "Found debian-release, using timestamp in this file."
        . ./debian-release
        TIMESTAMP=${BUILD_ID}
    fi
...
main()
{
	if [ ! -z "${CREATE_TARBALL}" ]; then
		echo "vimer 00 in main in mkrootfs.sh"
		make_rootfs_tarball ${CREATE_TARBALL}
		 echo "after make_rootfs_tarball"
		exit 0
	fi
